...
const DATASET_PATH = path.resolve(process.cwd(), "public/products_dataset/sagecart_products.json");
const LOW_STOCK_THRESHOLD = 5;
const BULK_BATCH_SIZE = 500;
const SOURCES_TO_REMOVE = ["myntra-dataset"];
...
const buildDatasetMeta = (item) => {
  ...
  return {
    source: cleanText(item.source) || "myntra-dataset",
    sourceOrigin: cleanText(item.sourceOrigin) || cleanText(item.datasetMeta?.sourceOrigin) || "combined-dataset",
...
const buildProductDoc = (item, userId, index) => {
  const source = cleanText(item.source) || "myntra-dataset";
  ...
  if (!pricing) {
    throw new Error(`Invalid price values for dataset row ${index + 1}`);
  }
...
const main = async () => {
  try {
    ...
    if (!fs.existsSync(DATASET_PATH)) {
      throw new Error(`Mapped combined dataset not found at ${DATASET_PATH}`);
    }

    const raw = JSON.parse(fs.readFileSync(DATASET_PATH, "utf8"));
    if (!Array.isArray(raw) || raw.length === 0) {
      throw new Error("Mapped combined dataset is empty.");
    }

    await connectDB();

    const removed = await Product.deleteMany({ source: { $in: SOURCES_TO_REMOVE } });
    console.log(`Removed ${removed.deletedCount || 0} existing combined-dataset products before reseeding.`);
    ...
    console.log(`Imported ${raw.length} combined products into SageCart.`);
    ...
    preview.forEach((product, index) => {
      console.log(
        `${index + 1}. ${product.name} | ${product.category} | stored ${product.price}/${product.offerPrice} USD-equivalent | source INR ${product.datasetMeta.sourcePriceInr}/${product.datasetMeta.sourceOfferPriceInr}`
      );
    });
