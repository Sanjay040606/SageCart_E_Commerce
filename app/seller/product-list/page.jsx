                ...
                <tbody className="text-sm text-gray-500">
                  {products.map((product, index) => (
                    <tr key={index} className="border-t border-gray-500/20">
                      ...
                      <td className="px-4 py-3">{formatPrice(convertUSDToINR(product.offerPrice), '₹')}</td>
                      <td className="px-4 py-3">{product.stock ?? 0}</td>
                      <td className="px-4 py-3 capitalize">{product.status || (product.stock === 0 ? 'out_of_stock' : 'active')}</td>
                      <td className="px-4 py-3 max-sm:hidden">
                        <button onClick={() => router.push(`/product/${product._id}`)} className="brand-button flex items-center gap-1 px-1.5 md:px-3.5 py-2 rounded-md">
