...
const RETURN_PICKUP_DELAY_DAYS = 1
const RETURN_RECEIPT_DELAY_DAYS = 1
...
export const hasReturnFlow = (order) =>
  RETURN_FLOW_STATUSES.has(order?.status) ||
  [
...
export const canLeaveReview = (order) => {
  if (!order?.deliveredAt) return false
  if (hasCanceledFlow(order)) return false

  return true
}

export const getReturnMilestones = (order) => {
  const returnConfirmedAt = getStatusTimestamp(
    order,
    ORDER_STATUSES.RETURN_CONFIRMED,
    order.returnRequestedAt || (hasReturnFlow(order) ? (order.deliveredAt || order.date) : null)
  );
  if (!returnConfirmedAt) return null;
  const returnStarted = new Date(returnConfirmedAt);
  
  const outForPickupEta = new Date(returnStarted.getTime() + (RETURN_PICKUP_DELAY_DAYS * DAY_IN_MS));
  outForPickupEta.setHours(8, 0, 0, 0);

  const returnedEta = new Date(returnStarted.getTime() + (RETURN_RECEIPT_DELAY_DAYS * DAY_IN_MS));
  returnedEta.setHours(18, 0, 0, 0);

  const returnedReference = getStatusTimestamp(order, ORDER_STATUSES.RETURNED, returnedEta);
  const refundCompletedEta = new Date(new Date(returnedReference).getTime() + DAY_IN_MS);
  refundCompletedEta.setHours(18, 0, 0, 0);

  return { returnStarted, outForPickupEta, returnedEta, refundCompletedEta };
}
...
export const syncOrderWithSystemTime = (order, now = new Date()) => {
  ...
  if (order.refundCompletedAt && order.status !== ORDER_STATUSES.REFUNDED) {
    order.status = ORDER_STATUSES.REFUNDED
    changed = addTimelineEntry(
      order,
  ...
  if (hasReturnFlow(order)) {
    ...
    if (returnMilestones) {
       const { outForPickupEta, returnedEta, refundCompletedEta } = returnMilestones;
    ...
    if (hasTimelineEntry(order, ORDER_STATUSES.RETURNED)) {
      const returnedTime = getStatusTimestamp(order, ORDER_STATUSES.RETURNED, order.returnRequestedAt)
      const refundReadyAt = returnedTime ? new Date(new Date(returnedTime).getTime() + DAY_IN_MS) : refundCompletedEta
      ...
      if (isPrepaidOrder(order)) {
        if (!order.refundRequestedAt) {
          ...
          changed = addTimelineEntry(
            ...
            new Date(order.refundRequestedAt),
            'Refund initiated for your return.'
          ) || changed
        }

        if (order.refundRequestedAt && now >= refundReadyAt) {
          order.refundCompletedAt = refundReadyAt
