const supportEmail = process.env.CONTACT_RECEIVER_EMAIL || "sagecart.support@gmail.com";

const baseTemplate = ({ title, preview, body, footer = "SageCart Team", supportLine = `If you have any queries, contact ${supportEmail}.` }) => `
  <div style="font-family: Arial, sans-serif; background:#f6f2ea; padding:24px; color:#314032;">
    <div style="max-width:680px; margin:0 auto; background:#fffdf8; border:1px solid #d8d3c7; border-radius:24px; overflow:hidden;">
      ...
      <div style="padding:18px 28px; border-top:1px solid #d8d3c7; color:#7f897a; font-size:13px;">
        <div>${footer}</div>
        ${supportLine ? `<div style="margin-top:6px; color:#657161;">${supportLine}</div>` : ""}
      </div>
    </div>
  </div>
`;
...
const getFeedbackPageLabel = (page) => {
  const labels = {
    home: "Home page",
    about: "About page",
...
export const buildFeedbackEmail = ({ name, email, page, subject, message }) => {
  const pageLabel = getFeedbackPageLabel(page);
  const subjectLabel = subject || pageLabel;

  return {
    subject: `[SageCart Feedback] ${subjectLabel}`,
    html: baseTemplate({
      title: "Feedback received",
      preview: `A signed-in user shared feedback from ${pageLabel}.`,
      body: `
        <p><strong>Name:</strong> ${name}</p>
        <p><strong>Email:</strong> ${email}</p>
        <p><strong>Source page:</strong> ${pageLabel}</p>
        <p><strong>Subject:</strong> ${subjectLabel}</p>
        <p><strong>Message:</strong></p>
        <div style="padding:16px; background:#f8f4ec; border-radius:16px; border:1px solid #d8d3c7;">${message}</div>
      `,
      footer: "Sent from a signed-in SageCart account.",
      supportLine: null,
    }),
    text: `${name}\n${email}\n${pageLabel}\n${subjectLabel}\n\n${message}`,
  };
...
export const buildReturnConfirmedEmail = ({ userName, orderId, pickupDate, items = [] }) => ({
  subject: `Return request confirmed for order ${orderId}`,
  html: baseTemplate({
    title: "Return Request Confirmed",
    preview: `Return request for order ${orderId} has been confirmed.`,
    body: `
      <p>Hi ${userName},</p>
      <p>Your return request for order <strong>${orderId}</strong> has been confirmed.</p>
      <p>The pickup is scheduled for <strong>${pickupDate}</strong>.</p>
      ${items.length ? listItems(items) : ""}
      <p style="margin-top:18px;">After pickup, we will process the return and the refund according to your payment method.</p>
    `,
  }),
});

export const buildReturnedEmail = ({ userName, orderId, returnedDate, refundDate, paymentMethod, items = [] }) => ({
  subject: `Your return for order ${orderId} was completed`,
  html: baseTemplate({
    title: "Return Completed",
    preview: `Order ${orderId} has been returned successfully.`,
    body: `
      <p>Hi ${userName},</p>
      <p>Your return for order <strong>${orderId}</strong> was completed on <strong>${returnedDate}</strong>.</p>
      ${items.length ? listItems(items) : ""}
      <div style="margin-top:18px; padding:16px; background:#f8f4ec; border:1px solid #d8d3c7; border-radius:16px;">
        <p style="margin:0 0 8px;"><strong>Payment Method:</strong> ${paymentMethod}</p>
        <p style="margin:0;">${refundDate ? `<strong>Refund ETA:</strong> ${refundDate}` : "Your refund will be processed according to the payment method used for the order."}</p>
      </div>
      <p style="margin-top:18px;">If the refund has already completed, you will receive a separate refund confirmation email.</p>
    `,
  }),
});
