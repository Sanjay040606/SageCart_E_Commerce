...
const isManualSellerProduct = (product = {}) =>
  cleanText(product?.source || product?.datasetMeta?.source).toLowerCase() === "manual";

const getParentProductStock = (product = {}) => {
  const numericStock = Number(product?.stock);
  return Number.isFinite(numericStock) ? Math.max(0, numericStock) : null;
};
...
const normalizeVariantOption = (option, product, index) => {
  ...
  return {
    ...
    offerPriceInr: Number.isFinite(Number(priceSource)) ? Number(priceSource) : null,
...
const getStorageVariantPricingKey = (option = {}) => {
  const variantType = cleanText(option?.type ?? option?.kind).toLowerCase();
  const directStorage = cleanText(option?.storage);
  if (directStorage) return directStorage.toLowerCase();

  if (variantType === "storage") {
    const labelSegments = cleanText(option?.label)
      .split("/")
      .map((part) => cleanText(part))
      .filter(Boolean);

    if (labelSegments.length >= 2) {
      return labelSegments.slice(-2).join("/").toLowerCase();
    }
  }

  const storageParts = [
    cleanText(option?.capacity),
    cleanText(option?.memory),
  ...
  return storageParts[0]?.toLowerCase() || "";
};

const backfillStorageVariantPricing = (options = []) => {
  if (!Array.isArray(options) || options.length === 0) return options;

  const pricingByStorage = new Map();

  options.forEach((option) => {
    const key = getStorageVariantPricingKey(option);
    if (!key) return;

    const offerPrice = Number(option?.priceInr ?? option?.offerPriceInr);
    const originalPrice = Number(option?.originalPriceInr);
    const entry = pricingByStorage.get(key) || { priceInr: null, originalPriceInr: null };

    if (!(Number.isFinite(entry.priceInr) && entry.priceInr > 0) && Number.isFinite(offerPrice) && offerPrice > 0) {
      entry.priceInr = offerPrice;
    }

    if (!(Number.isFinite(entry.originalPriceInr) && entry.originalPriceInr > 0) && Number.isFinite(originalPrice) && originalPrice > 0) {
      entry.originalPriceInr = originalPrice;
    }

    pricingByStorage.set(key, entry);
  });
  ...
  return options.map((option) => {
    ...
    const entry = pricingByStorage.get(key);
    if (!entry) return option;

    const next = { ...option };
    const hasOfferPrice = Number.isFinite(Number(next.priceInr ?? next.offerPriceInr)) && Number(next.priceInr ?? next.offerPriceInr) > 0;
    const hasOriginalPrice = Number.isFinite(Number(next.originalPriceInr)) && Number(next.originalPriceInr) > 0;

    if (!hasOfferPrice && Number.isFinite(entry.priceInr) && entry.priceInr > 0) {
      next.priceInr = entry.priceInr;
      next.offerPriceInr = entry.priceInr;
    }

    if (!hasOriginalPrice && Number.isFinite(entry.originalPriceInr) && entry.originalPriceInr > 0) {
      next.originalPriceInr = entry.originalPriceInr;
    }

    if (
      Number.isFinite(Number(next.priceInr)) &&
      Number.isFinite(Number(next.originalPriceInr)) &&
      Number(next.priceInr) > 0 &&
      Number(next.originalPriceInr) > 0 &&
      Number(next.originalPriceInr) < Number(next.priceInr)
    ) {
      const swappedOffer = Number(next.originalPriceInr);
      const swappedOriginal = Number(next.priceInr);
      next.priceInr = swappedOffer;
      next.offerPriceInr = swappedOffer;
      next.originalPriceInr = swappedOriginal;
    }
...
export const isVariantUnavailable = (option = {}) =>
  Boolean(
    option &&
      (option.outOfStock ||
...
export const buildProductVariantOptions = (product) => {
  ...
  if (normalizedExplicit.length > 0) {
    const dedupedOptions = backfillStorageVariantPricing(dedupeVariantOptions(normalizedExplicit));
    const parentStock = getParentProductStock(product);
    const hasManualStockFallback =
      isManualSellerProduct(product) &&
      parentStock !== null &&
      parentStock > 0 &&
      dedupedOptions.every((option) => isVariantUnavailable(option));

    if (hasManualStockFallback) {
...
export const getProductAvailabilitySummary = (product = {}) => {
  const rawStatus = cleanText(product?.status).toLowerCase();
  const numericStock = Number(product?.stock);
  const normalizedStock = Number.isFinite(numericStock) ? Math.max(0, numericStock) : null;
  const variantOptions = buildProductVariantOptions(product);
  const availableVariantCount = variantOptions.length
    ? variantOptions.filter((option) => !isVariantUnavailable(option)).length
    : null;

  if (rawStatus === "inactive") {
    return {
      status: "inactive",
      stock: normalizedStock ?? 0,
      variantCount: variantOptions.length,
      availableVariantCount
    };
  }

  if (variantOptions.length > 0 && availableVariantCount === 0) {
    return {
      status: "out_of_stock",
      stock: 0,
      variantCount: variantOptions.length,
  ...
  if (rawStatus === "out_of_stock" || rawStatus === "low_stock") {
    return {
      status: rawStatus,
      stock: 0,
      variantCount: variantOptions.length,
...
export const getVariantDisplayPriceInr = (product, variant) => {
  const explicitPrice = Number(variant?.priceInr ?? variant?.offerPriceInr);
...
export const getVariantDisplayOriginalPriceInr = (product, variant) => {
  const explicitOriginalPrice = Number(variant?.originalPriceInr);
