...
const isManualProduct = (product = {}) =>
  normalizeKey(product?.source || product?.datasetMeta?.source) === "manual";
...
export const buildStockUpdateOperations = (products = [], options = {}) => {
  const eligibleProducts = products.filter((product) => !isManualProduct(product));
  const skippedManualCount = products.length - eligibleProducts.length;
  ...
  const grouped = groupProductsByCategory(eligibleProducts);
