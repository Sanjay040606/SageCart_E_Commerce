...
export const getOrderMilestones = (order) => {
  ...
  return { placedAt, shippedEta, deliveryEta, returnDeadline }
}
...
export const syncOrderWithSystemTime = (order, now = new Date()) => {
  ...
  let changed = false
  const { placedAt, shippedEta, deliveryEta, returnDeadline } = getOrderMilestones(order)
  ...
  if (hasCanceledFlow(order)) {
    ...
    if (
      ...
      !hasTimelineEntry(order, ORDER_STATUSES.REFUNDED)
    ) {
  ...
  // Calculate delivery completion time as 6pm on delivery date
  const deliveryCompletionTime = new Date(deliveryEta)
  deliveryCompletionTime.setHours(18, 0, 0, 0) // Set to 6:00 PM on delivery date

  if (now >= deliveryCompletionTime && !order.deliveredAt) {
    order.deliveredAt = deliveryCompletionTime
    changed = true
  }

  if (order.deliveredAt && !hasTimelineEntry(order, ORDER_STATUSES.OUT_FOR_DELIVERY)) {
    changed = addTimelineEntry(order, ORDER_STATUSES.OUT_FOR_DELIVERY, new Date(order.deliveredAt), 'Your order is out for delivery today.') || changed
  }
